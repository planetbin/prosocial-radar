...
def _markdown_digest_block(digest_markdown: str) -> str:
    if not digest_markdown or not digest_markdown.strip():
        return ""
    return (
        '<div style="margin:24px 0 20px;padding:18px 20px;background:#fff;'
        'border:1px solid #cbd5e1;border-radius:8px;">'
        '<h2 style="margin:0 0 10px;font-size:16px;color:#0f172a;">Markdown Literature Brief</h2>'
...
def render_email(papers: List[Dict], total_found: int, digest_markdown: str = "") -> str:
    today = date.today().strftime("%B %d, %Y")
    cards_html = "".join(_paper_card(i + 1, p) for i, p in enumerate(papers))
    digest_html = _markdown_digest_block(digest_markdown)
...
def send_email(papers: List[Dict], total_found: int, digest_markdown: str = "") -> bool:
    ...
    html_body = render_email(papers, total_found, digest_markdown=digest_markdown)
