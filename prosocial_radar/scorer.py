...
Computes a relevance_score (0-100) for each paper based on:
  - Topic relevance tier and direct anchor matches          -> up to 55 pts
  ...
  - Adjacent/noisy-topic penalty                           -> up to -12 pts
"""
...
def _topic_score(paper: Dict) -> float:
    ...
    anchor_bonus = min(len(core) * 3.0, 10.0) + min(len(paradigms) * 4.0, 8.0)
    mechanism_bonus = min(len(mechanisms) * 1.5, 6.0) if tier == "mechanism_linked" else 0.0
    return min(base + anchor_bonus + mechanism_bonus, 55.0)
